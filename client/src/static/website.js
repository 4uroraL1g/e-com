const categories = [
    {
        ...
        products: [
              ...
              { id: 21, name: "Oleb Bed", price: 3000000, height: "128cm", width: "210cm", depth: "59cm", img: "https://m.media-amazon.com/images/I/81VzkftpjOL.jpg", url: "https://m.media-amazon.com/images/I/81VzkftpjOL.jpg" }
        ]
},
   {
    name: "Sofas",
...
function searchProduct() {
    ...
    const productsContainer = document.getElementById("productsContainer") || document.getElementById("categories");
  
    if (!productsContainer) {
      alert("No product container found on this page.");
      return;
    }
    ...
    if (!query) {
      // No search query: show all categories and their products (for main page)
      if (productsContainer.id === "categories") {
        categories.forEach(category => {
          ...
          `;
          productsContainer.innerHTML += categoryHTML;
        });
      } else {
        // On category page, show all products for that category
        const currentCategoryName = document.querySelector("h2")?.textContent.trim();
        const category = categories.find(cat => cat.name.toLowerCase() === currentCategoryName.toLowerCase());
    ...
    // With query: filter products by name
    const matchedProducts = [];
    categories.forEach(category => {
      category.products.forEach(product => {
        if (product.name.toLowerCase().includes(query)) {
          matchedProducts.push({ ...product, category: category.name });
        }
      });
    });
    ...
    if (productsContainer.id === "productsContainer") {
      // Category page: just show matched products
      matchedProducts.forEach(product => {
        productsContainer.innerHTML += createProductHTML(product);
      });
    } else {
      // Main page: group matched products by category and show only those categories
      const grouped = {};
      ...
      for (const catName in grouped) {
        let categoryHTML = `
          <div class="category">
            <h2>${catName}</h2>
            <div class="products">
              ${grouped[catName].map(product => createProductHTML(product)).join("")}
            </div>
          </div>
        `;
        productsContainer.innerHTML += categoryHTML;
