...
const ProductUpload = () => {
  ...
  useEffect(() => {
    window.scrollTo(0, 0);
    setCountryList([{ value: "All", label: "All" }, ...context?.countryList]);
  ...
  const handleRemoveImage = (index, imgUrl) => {
    deleteImages(`/api/products/deleteImage?img=${imgUrl}`).then(() => {
  ...
  const handleSubmit = (e) => {
    ...
    setIsLoading(true);
    postData("/api/products/create", { ...formFields, images: previews }).then(
      () => {
        context.setAlertBox({
          open: true,
          error: false,
              ...
              {uploading ? (
                ...
                <>
                  <input
                    type="file"
                    multiple
                    onChange={(e) => handleFileUpload(e, "/api/products/upload")}
                  />
                  <div className="info">
                    <FaRegImages />
                    <h5>Image Upload</h5>
